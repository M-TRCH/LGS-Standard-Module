...
- **A stream timeout bounds ONE BYTE, not one read.** `Stream::readBytes` calls `timedRead` per byte and each call restarts `_startMillis`, so the 29 ms that looked like the read budget was really 29 ms x however many bytes libmodbus asked for — up to ~250 when a garbage byte is parsed as a function code, which is 7 s in a single tick and straight past the 4 s watchdog. `rs485PortBegin` sets 2 ms flat since v3.3.2: `modbusServerTick`'s frame-gap gate means a real frame is already whole in the ring before `poll()`, so waiting for future bytes is never useful — they belong to the next frame. (This was NOT the cause of the bench cabinet's whole-channel IWDG resets, which were wiring; it is a bound worth having anyway.)
