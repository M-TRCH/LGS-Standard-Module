...
- **PULSE width**: จ่ายไฟ**อย่างน้อย** `LATCH_PULSE_MS` (300ms) ไม่ว่า sense เป็นอะไร; หลังครบ 300ms
  ถ้า sense ยัง**ล็อก** (LOW) จ่ายต่อจนกลอนเปิด (sense HIGH) หรือชนเพดาน `LATCH_MAX_UNLOCK_TIME`
  (500ms) — เงื่อนไขจบ = `elapsed ≥ 500 || (elapsed ≥ 300 && sense เปิด)` (มีผลทุก trigger:
  Modbus coil 1020/1021 + DEMO test); เข้า PULSE เฉพาะเมื่อ sense ล็อกตอนเริ่ม (START guard)
...
- **RUN**: ยังไม่ใช้ OLED (จอว่าง; ถ้าเข้ามาจากการกดปุ่มค้างแล้วปล่อย จอจะค้างคำว่า "RUN" จาก selector)
- **DEMO**: ตัวนับ 0–99 เลขใหญ่ (`oledPrintLargeNumber` → GFXfont `OledBigNum`, เริ่มที่เลข ID) เพิ่มด้วยการแตะปุ่ม — ไว้ทดสอบจอก่อนเปิดใช้ RUN จริง; **แตะปุ่ม = สั่งทดสอบปลดล็อกกลอนด้วย** (`latchRequestUnlock`, pulse เฉพาะเมื่อยังล็อก, safety ครบ)
