...
L4  app/        app (boot + tick pipeline) · modes · led_control (8 presets) ·
                latch_control · ops · display_control (จอ RUN) · servo_control (stub)
                — include ได้ทุกชั้น
...
- **Hardware guard (TIM7)**: ตอนเข้า PULSE จะ arm one-shot timer ISR ที่บังคับ MOSFET LOW
  เมื่อครบ **เพดาน 500ms** (`LATCH_MAX_UNLOCK_TIME`) — hard backstop ไม่ขึ้นกับ cadence ของ loop
  (poll() ของ Modbus block ได้หลายร้อย ms ระหว่าง flush response ยาวหรือเจอ noise บนบัส)
- **sense (PA3)**: active-low (LOW = ล็อก), ตั้งเป็น `INPUT` (บอร์ด pull ภายนอกเอง ไม่ใช้ internal pull-up)
...
**Schema v2** (ปัจจุบัน): `identifier, baudRate, unlockDelayMs + LedPreset{brightness,r,g,b,maxOnTimeS}[8]`
= 43×uint16 = 86B (blob 96B) — `identifier` ต้องอยู่ offset 0 เสมอ (เส้นกู้ torn-blob พึ่งตำแหน่งนี้
ข้ามทุก schema). บูตแรกที่เจอ blob v1 (CRC ผ่าน) จะ migrate ในที่: ค่าเดิม → preset 1,
preset 2–8 = palette default, เขียน payload ก่อน header (torn กลาง migration = เข้าเส้นกู้ identifier)
...
## LED presets (app/led_control)

วงแหวน 16 พิกเซลวงเดียว, **8 color presets** (config ราย preset ที่ regs `100+10n..+4`, persist ทั้งหมด):
- **Radio switching**: coil enable `1000+n` — เปิด preset ใหม่ = เปลี่ยนสีทันที + เคลียร์ coil
  (enable + display-combo) ของตัวเก่าผ่าน `mbCoilWrite` (shadow sync → ไม่ re-fire handler);
  อ่าน coils เห็น preset ที่ติดจริงตัวเดียวเสมอ
- **Combo families**: `1010+n` = preset+display (state, mirror enable ทันที) · `1020+n` =
  preset+latch (command, Safety; enable coil sync หลัง pulse ผ่าน `finishRequest` เมื่อยังเป็น
  preset ที่ active อยู่ — `ledControlActiveEnableCoil()`) · `1030+n` = preset+latch+display
  (command, R5.0-new; จอติดตอนรับคำสั่ง)
- Max-on-time บังคับตาม reg ของ preset ที่ติดอยู่; สถิติราย preset publish ที่ `200+10n/201+10n`,
  ยอดรวมที่ 200/201; global 190/194 fan-out เขียนทุก preset
...
ทั้งชุด (`__aeabi_d*/f*`, `_dtoa`)

Tier 3: เลขใหญ่ DEMO เปลี่ยนจาก bitmap 53×64 fixed (`kOledDigits`, 4,480 B) → GFXfont
`OledBigNum` (trim ขอบว่าง + bit-pack, พิกเซลเหมือนเดิมเป๊ะ) สร้างด้วย `tools/gen_oled_bignum.py`
(มี round-trip pixel check). รวม 3 tier: **64,792 → 54,928 B (−9,864, −15.2%)**
...
- Hardware gates ค้างอยู่ (ต้องบอร์ดจริงก่อน release):
  ...
  (b) Latch: วัด pulse — กลอนเปิดเร็ว→ON 300ms เต็ม; เปิดช้า→ยืดจนเปิด; ไม่เปิด→ตัดที่ 500ms; cooldown, Modbus ตอบระหว่าง pulse
