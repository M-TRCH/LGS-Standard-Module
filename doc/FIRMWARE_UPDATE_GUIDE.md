...
### แนะนำ: LGS Test Tool แท็บ "ติดตั้งโมดูลใหม่" (flash + ตั้ง ID ครั้งเดียวจบ)

ตั้งแต่ firmware v3.1.0 ตัว image พก **commissioning block** มาด้วย เครื่องมือจะใส่
Modbus ID ลงในไฟล์ก่อนเขียน โมดูลจึงตอบที่หมายเลขนั้นตั้งแต่บูตครั้งแรก —
**ไม่ต้องต่อบัส RS485 และไม่ต้องตั้ง ID แยกอีกรอบ**

1. ต่อ ST-Link เข้าโมดูล (ไม่ต้องต่อสายสื่อสาร)
2. เปิด LGS Test Tool → แท็บ **ติดตั้งโมดูลใหม่**
3. เลือกไฟล์ `*_factory_*.bin` → กรอก Slave ID (กดจากผังได้) → กด **เขียนและตั้ง ID**

ทุกครั้งที่ทำจะต่อท้าย `data/exports/commission_log.csv` พร้อม **UID ของชิป**
(เลขประจำตัวจากโรงงาน อ่านผ่าน SWD) ล็อตการผลิต และเวลา — ใช้เป็นบันทึกสืบย้อนได้
โดยที่ทุกบอร์ดยังใช้ไบนารีไฟล์เดียวกัน ซึ่งเป็นเงื่อนไขที่ OTA ต้องการ

**กติกาความปลอดภัย:** ถ้าโมดูลมี ID อยู่แล้ว ไฟล์จะ**ไม่แตะ**เลย ต้องติ๊ก
"เขียนทับ ID ที่โมดูลเก็บไว้แล้ว" เท่านั้นจึงจะเปลี่ยนได้ ไฟล์ที่ใส่ ID ไว้จึง
เปลี่ยนเลขตู้ที่ใช้งานอยู่ไม่ได้ และ **image ที่ส่งผ่าน OTA เปลี่ยน ID ไม่ได้เลย**
เพราะไม่เคยถูกใส่ค่า

โมดูลจำว่ากินไฟล์ไหนไปแล้ว จึง flash ไฟล์เดิมซ้ำกี่ครั้งก็ไม่ย้อน ID ที่เปลี่ยนทีหลัง
ทาง Modbus และ **factory reset ก็ยังคืนค่าเป็น 247 จริง** ไม่ถูกตั้ง ID กลับเงียบๆ

> สร้างไฟล์ factory เองด้วย `python tools/make_factory_image.py`
> (ต้อง `pio run -e LGS_BOOT` และ `pio run` ก่อน) สคริปต์ตรวจให้ด้วยว่า image
> มี commissioning block ครบและมีจุดเดียว
