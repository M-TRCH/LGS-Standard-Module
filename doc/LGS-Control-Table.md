...
| 61 | Lit Windows (bitmask) | R | 0 | 0-255 | - | | | | ✓ ¹⁵ |
| 62–63 | *(สำรอง)* | | | | | | | | |
...
⁹ เรนเดอร์จริงบน OLED เมื่อ coil 1010 = 1 ด้วยฟอนต์เลขใหญ่ — **ช่วงที่แสดงคือ 0-999 ตั้งแต่ fw v3.4.0** (v3.3.x และก่อนหน้า clamp ที่ 99): ค่าที่เขียนเกินช่วงจะถูก clamp และรีจิสเตอร์สะท้อนค่าที่ clamp แล้ว; ค่าต่ำกว่า 100 แสดง 2 หลักตามเดิม ("45" ไม่ใช่ "045") ขนาดตัวเลขเท่ากันทุกกรณี; เขียนค่าใหม่ขณะจอเปิดอยู่ → อัปเดตทันที; ค่า volatile (รีเซ็ตเป็น 0 เมื่อรีบูต)
...
¹⁵ ใหม่ใน fw v3.5.0 (เดิมเป็นช่องสำรอง): bit n-1 = window/preset n ติดอยู่ — ความจริงแบบหลายหน้าต่างของบอร์ด mask (type 10) ที่ reg 11 ค่าเดียวบอกไม่ได้; บนบอร์ดวงแหวนสะท้อน active preset (0 หรือบิตเดียว) · อัปเดตทุก 1 วินาทีพร้อมกลุ่ม Diagnostics
...
¹³ พฤติกรรมขึ้นกับจอที่บอร์ดมี (ตัดสินครั้งเดียวตอนบูตจาก device type):
   · **บอร์ดวงแหวน (type 20 และ ring ทุกแบบ) — Radio switching**: coil ตระกูล Light 1–8 ทั้งหมดขับ**วงแหวนเดียวกัน** โดยเลือก preset สี — เปิด preset ใหม่ขณะตัวเก่าติดอยู่ = วงแหวนเปลี่ยนสีทันทีและ coil ของตัวเก่า (ทั้ง enable และ display-combo) ถูกเคลียร์เป็น 0 อัตโนมัติ; อ่าน coils จะเห็นตัวที่ติดจริงตัวเดียวเสมอ
   · **บอร์ด mask 8 ช่อง (type 10 STANDARD, fw ≥ v3.5.0) — หน้าต่างอิสระ**: coil แต่ละตัวคุมหน้าต่างของตัวเองเท่านั้น เปิด-ปิดพร้อมกันกี่ช่องก็ได้ (window n = ผู้ป่วย/ผู้จัดยาคนที่ n สูงสุด 8 คนต่อช่องยา — semantics เดียวกับ Light 1–8 แยกขาบน R4.x); max-on-time (114/…/184) บังคับแยกรายหน้าต่าง; อ่านชุดที่ติดทั้งหมดได้จาก reg 61 (bitmask) — reg 11 บอกตัวล่าสุดที่สั่งขณะยังติด · fw ≤ v3.4.0 บนบอร์ด mask ยังเป็น radio
